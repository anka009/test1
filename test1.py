...
st.title("🧬 Iterative Kern-Zählung — Version 3 (Kontur-basiert + Live-Vorschau)")
...
with col2:
    ...
    calib_radius=st.sidebar.slider("Kalibrier-Radius (px, original)",1,30,5)
    min_area_display=st.sidebar.number_input("Min. Konturfläche (px) — Display",min_value=1,max_value=1000,value=8)
    ...
    try:
        ...
        hema_vec0=np.array([0.65,0.7,0.29])
...
if coords:
    ...
    if mode=="Punkt löschen":
        ...
        if vec is None:
            ...
            centers_orig,mask=detect_centers_contours(channel_full,min_area_orig)
            new_centers=dedup_new_points(centers_orig,st.session_state.all_points,dedup_dist_orig)

            # --- Live Vorschau der Maske ---
            if mask is not None:
                mask_disp=cv2.resize(mask,(DISPLAY_WIDTH,H_disp),interpolation=cv2.INTER_NEAREST)
                mask_color=cv2.cvtColor(mask_disp,cv2.COLOR_GRAY2BGR)
                mask_color[:,:,1:]=0
                preview=cv2.addWeighted(display_canvas,0.7,mask_color,0.3,0)
                st.image(preview,caption="🔴 Vorschau erkannte Kerne",use_column_width=True)
            ...
            if new_centers:
                ...
                st.info("Keine neuen Kerne (alle gezählt oder keine Detektion).")
...
st.markdown("---")
st.caption("Hinweise: Deconvolution erfolgt auf Originalbild. Live Vorschau zeigt erkannten Kern-Masken in rot. "
